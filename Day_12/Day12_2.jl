...
function get_all_props(g)
  res = []
  for v in vertices(g)
    push!(res, props(g, v)[:name])
  end
  return res
end

function bf_search(g, start, visited, visit_2)
  n_paths = 0
  for n in neighbors(g, start)
    if props(g, n)[:name] == "end"
      n_paths += 1
    elseif props(g, n)[:name] == "start"
    elseif get(visited, n, 0) > 0 && islowercase(props(g, n)[:name][1]) && !visit_2
        tmp_visisted = copy(visited)
        tmp_visisted[n] = get(tmp_visisted, n, 0) + 1
        n_paths += bf_search(g, n, tmp_visisted, true)
    elseif get(visited, n, 0) > 0 && islowercase(props(g, n)[:name][1])
    else 
      tmp_visisted = copy(visited)
      tmp_visisted[n] = get(tmp_visisted, n, 0) + 1
      n_paths += bf_search(g, n, tmp_visisted, visit_2)
    end
  end
  return n_paths
end
...
function start_search(g, start)
  ...
  for n in neighbors(g, start)
    ...
    res += bf_search(g, n, visited, false)
  end
  return res
end
